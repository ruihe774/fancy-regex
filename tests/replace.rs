...
fn replacer_string() {
    ...
    let result = regex.replace("a sucessful b", "${1}ucce$2").unwrap();
    assert_eq!(result, "a successful b");

    // Replacer impl for &String
    let repl_string = "${1}ucce$2".to_string();
    let result = regex.replace("a Suceeded b", &repl_string).unwrap();
    assert_eq!(result, "a Succeeded b");

    // Replacer impl for String
    let result = regex.replace("a sucessor b", repl_string).unwrap();
...
fn replacer_cow() {
    ...
    let result = regex
        .replace("a ommiting b", &Cow::from("${1}mit$2"))
        .unwrap();
    assert_eq!(result, "a omitting b");

    // Replacer for Cow<str>::Borrowed
    let result = regex
        .replace("a ommited b", Cow::Borrowed("${1}mit$2"))
        .unwrap();
    assert_eq!(result, "a omitted b");

    // Replacer for Cow<str>::Owned
    let result = regex
        .replace("a Ommits b", Cow::Owned("${1}mit$2".to_string()))
        .unwrap();
...
fn replacer_noexpand() {
    ...
    let result = regex
        .replace("a anullment b", NoExpand("${1}nnul$2"))
        .unwrap();
...
fn replacer_callback() {
    ...
    let result = regex
        .replace("a Aprox b", |cap: &Captures| {
...
fn replace_one() {
    ...
    assert_eq!(regex.replace("blabla", "foo").unwrap(), "foobla");
}
...
fn replace_all() {
    ...
    assert_eq!(
        regex.replace_all("aaaa aaa aa a", "xx").unwrap(),
        "xxxx xxa xx a"
    );
}
...
fn replacen() {
    ...
    assert_eq!(
        regex
            .replacen("blablabla", Some(NonZeroUsize::new(2).unwrap()), "foo")
            .unwrap(),
        "foofoobla"
    );
}
